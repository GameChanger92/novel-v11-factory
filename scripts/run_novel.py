...
import os, re, csv, json, textwrap, pathlib, dotenv, openai, random, time
dotenv.load_dotenv()
...
BIBLE = pathlib.Path(f"projects/{PROJECT_NAME}/story_bible.yaml").read_text(encoding="utf8") # <--- 이렇게 수정!
EP_DIR = pathlib.Path(f"projects/{PROJECT_NAME}/episodes");  EP_DIR.mkdir(exist_ok=True) # <--- 이렇게 수정!
SUM_DIR = pathlib.Path(f"projects/{PROJECT_NAME}/summaries"); SUM_DIR.mkdir(exist_ok=True) # <--- 이렇게 수정!
...
def scene_points(bit:dict) -> list[str]:
    ...
    prompt = ("이 줄거리를 6개 장면 포인트로 분해해줘."
              "각 항목은 1문장 한국어로, 순서 유지.")
...
def self_critique(text:str) -> str:
    ask = ("아래 소설을 읽고 문제점 3개와 수정본(동일 분량)을 줘."
...
def quick_guard(txt:str):
    ...
    if not (OUTLEN-4000 <= len(txt) <= OUTLEN+4000): return "LEN"
    if re.search(r"성훈[^.\n]{0,20}금발", txt):     return "CHAR_COLOR"
    return "OK"
...
# ───────── Main loop ─────────
def run(total:int):
    outline_map = {int(r["ep_no"]): r["outline"]
                   for r in csv.DictReader(open(f"projects/{PROJECT_NAME}/outline.csv",encoding="utf8"))}
    for n in range(1, total+1):
        ...
        if quick_guard(draft) != "OK":
            print(f"✗ EP{n}: guard fail"); continue
        ...
        print(f"✓ EP{n} saved"); time.sleep(random.uniform(1,3))
